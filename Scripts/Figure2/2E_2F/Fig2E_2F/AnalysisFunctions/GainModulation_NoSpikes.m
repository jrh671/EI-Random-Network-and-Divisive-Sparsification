...
% Amount of time to exclude around every spike
PRE_SPIKE_MS  = 500;
POST_SPIKE_MS = 500;
...
Rate_sec = squeeze(sum(spike_rs, 2));  

%% Exclude entire 1 s Vm windows containing any spike

spiking_window = Rate_sec > 0;

Vm_sec_nospike = Vm_sec_centered;
Vm_sec_nospike(spiking_window) = NaN;

fprintf('Fraction of neuron x 1-s Vm windows excluded: %.3f\n', ...
    mean(spiking_window(:)));
...
if any(idx_low)
    Vm_low(n,p) = mean(Vm_sec_nospike(n,idx_low), 'omitnan');
    Rate_low(n,p) = mean(Rate_sec(n,idx_low), 'omitnan');
end

if any(idx_high)
    Vm_high(n,p) = mean(Vm_sec_nospike(n,idx_high), 'omitnan');
