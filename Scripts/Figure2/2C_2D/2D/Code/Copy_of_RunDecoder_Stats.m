...
elseif Decoder == 3
    [decoding_error(N, L), sparsity(N,L),prediction,TestPos] = linear_decoder_with_l1(Spikes, integer_pos, lambda);
elseif Decoder ==4
    [decoding_error(N, L), sparsity,prediction,TestPos] = linear_decoder_with_l2(Spikes, integer_pos, lambda);
elseif Decoder ==5
...
if Instance<2
conditions = linspace(0,0.3,21); % Adjust this as per your conditions
elseif Instance>1
...
hErrorbar(I) = errorbar(conditions(Length), means(Length), std_errors(Length), '-', 'Color',  currentColor, 'HandleVisibility', 'off');
SaveMean{I,1}= means;
