...
firing_rates_CA3 = cellfun(@(x) max(x, 0), firing_rates_CA3, 'UniformOutput', false);
    ...
    % Check which neurons have activity above the threshold at the current position 
    active_neurons_CA3 = [];
...
if ~isempty(SpecialCA3)
            ...
            if time_bins_CA3(1, i) == 1 
                for j = 2:TLearning
                    ...
                    for k = later_crossings
                        if i ~= k && (~memory_neurons_CA3(i) || ~memory_neurons_CA3(k)) % Avoid self-recurrent connections
    ...
    % Apply decay to recurrent connections, excluding memory cell connections
    for i = 1:N
        for j = 1:N
...
% Normalize and threshold recurrent connections in CA3
