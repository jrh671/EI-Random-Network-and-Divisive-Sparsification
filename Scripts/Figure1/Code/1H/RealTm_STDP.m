...
theta_E = zeros(size(excit_spikes));   % same shape as excit_spikes
theta_I = zeros(size(inhib_spikes));   % same shape as inhib_spikes

theta_eta = 0.001;

% Before loop, initialize once
...
% Updating excitatory cumulative input 
    ...
    % Inside loop each time step
    E_accum = E_accum + excit_spikes;
    I_accum = I_accum + inhib_spikes;
    window_count = window_count + 1;
    
    if window_count == K

    
        % Average activity over last K time points
        E_avg = E_accum / K;
        I_avg = I_accum / K;
    
        % BCM sliding thresholds
        theta_E = (1 - theta_eta) * theta_E + theta_eta * (E_avg.^2);
        theta_I = (1 - theta_eta) * theta_I + theta_eta * (I_avg.^2);
    
        % BCM factors using averaged postsynaptic activity
        bcm_E = E_avg .* (E_avg - theta_E);   % 1 x N_E
        bcm_I = I_avg .* (I_avg - theta_I);   % 1 x N_I
...
hold on;
xline(theta,'--r','\theta_E');
yline(0,'k');

xlabel('Postsynaptic activity E_{avg}');
ylabel('BCM kernel  E(E-\theta_E)');
title('BCM postsynaptic kernel');
grid on;

 if Save==1
% Now you can save the results outside the parfor loop
    % Construct a unique filename for each iteration
    filename = sprintf('./SavedFiles/resultsFastBCM1.mat');
    % Save the results
    save(filename, 'results');

        % Construct a unique filename for each iteration
    filename2 = './SavedFiles/W_InputEFastBCM1.mat';
    % Save the results
    save(filename2, 'W_inputE');

    % Construct a unique filename for each iteration
    filename3 = './SavedFiles/PF_CellFastBCM1.mat';
    % Save the results
    save(filename3, 'pf_cell');

        % Construct a unique filename for each iteration
    filename4 = './SavedFiles/Mean_EIFastBCM1.mat';
    % Save the results
    save(filename4, 'Mean_EI');

    % Construct a unique filename for each iteration
    filename5 = './SavedFiles/Mean_IEFastBCM1.mat';
    % Save the results
