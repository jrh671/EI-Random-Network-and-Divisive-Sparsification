...
pf_width_ = 2.4495;                             % controls the Gaussian pf width
...
excit_cumul_input = zeros(n_excit, length(total_time_vec)); % stores excitatory spikes 
...
% Updating excitatory cumulative input 
    ...
    excit_cumul_input(:,tt) = excit_cum_input;
...
VmE=SlidingAverage_s_mean(excit_cumul_input,mean_dt/mean_v);
Positions=SlidingAverage_s_mean(positions,mean_dt/mean_v);
InputRates=SlidingAverage_s(Input_Spikes,mean_dt/mean_v);
